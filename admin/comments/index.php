...
if (!$tableExists) {
    ...
    <main class="gdy-admin-page">
      <div class="alert alert-warning">
        <b><?php echo h(__('t_558f159797', 'جدول التعليقات غير موجود.')); ?></b>
        <div class="mt-2"><?php echo h(__('t_a33e702b5e', 'نفّذ ملف SQL التالي مرة واحدة في phpMyAdmin:')); ?></div>
        <div class="mt-2">
...
<main class="gdy-admin-page">
  <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3">
    <div>
      <h1 class="h5 m-0"><?php echo h(__('t_422df4da8b', 'التعليقات')); ?></h1>
      <div class="text-muted small mt-1"><?php echo h(__('t_c7b339b071', 'إدارة التعليقات واعتمادها أو رفضها قبل النشر.')); ?></div>
    </div>

    <div class="d-flex gap-2 flex-wrap">
      <a class="btn btn-sm <?php echo $status==='pending'?'btn-primary':'btn-outline-primary'; ?>" href="?status=pending">قيد المراجعة (<?php echo (int)$counts['pending']; ?>)</a>
      <a class="btn btn-sm <?php echo $status==='approved'?'btn-success':'btn-outline-success'; ?>" href="?status=approved">معتمد (<?php echo (int)$counts['approved']; ?>)</a>
      <a class="btn btn-sm <?php echo $status==='rejected'?'btn-danger':'btn-outline-danger'; ?>" href="?status=rejected">مرفوض (<?php echo (int)$counts['rejected']; ?>)</a>
      <a class="btn btn-sm <?php echo $status==='all'?'btn-dark':'btn-outline-dark'; ?>" href="?status=all">الكل (<?php echo (int)$counts['all']; ?>)</a>
    </div>
  </div>
  ...
  <form class="row g-2 align-items-center mb-3" method="get">
    ...
    <div class="col-12 col-md-6">
      <input class="form-control form-control-sm" name="q" value="<?php echo h($q); ?>" placeholder="<?php echo h(__('t_c5d269b34b', 'بحث بالاسم/الإيميل/النص/عنوان الخبر')); ?>">
    </div>
    <div class="col-12 col-md-auto">
      <button class="btn btn-sm btn-outline-secondary"><?php echo h(__('t_ab79fc1485', 'بحث')); ?></button>
      <a class="btn btn-sm btn-outline-dark" href="?status=<?php echo h($status); ?>"><?php echo h(__('t_ec2ce8be93', 'مسح')); ?></a>
    </div>
  </form>
  ...
  <div class="card border-0 shadow-sm">
    <div class="table-responsive">
      <table class="table table-sm align-middle mb-0">
        <thead class="table-light">
          <tr>
            <th style="width:90px;"><?php echo h(__('t_1253eb5642', 'الحالة')); ?></th>
            <th><?php echo h(__('t_533ba29a76', 'التعليق')); ?></th>
            <th style="width:220px;"><?php echo h(__('t_213a03802a', 'الخبر')); ?></th>
            <th style="width:160px;"><?php echo h(__('t_901675875a', 'المرسل')); ?></th>
            <th style="width:140px;"><?php echo h(__('t_8456f22b47', 'التاريخ')); ?></th>
            <th style="width:90px;"><?php echo h(__("t_admin_spam", "سبام")); ?></th>
            <th style="width:220px;"><?php echo h(__('t_901efe9b1c', 'إجراءات')); ?></th>
          </tr>
        </thead>
        <tbody>
          <?php if (!$items): ?>
            <tr><td colspan="6" class="text-center text-muted py-4"><?php echo h(__('t_b4e385fa98', 'لا توجد تعليقات.')); ?></td></tr>
          <?php endif; ?>
          ...
          <?php foreach ($items as $c): ?>
            ...
            ?>
            <tr>
              <td><span class="badge bg-<?php echo h($badge); ?>"><?php echo h($st); ?></span></td>
              <td>
                <div class="fw-semibold"><?php echo h(mb_substr((string)$c['body'], 0, 140, 'UTF-8')); ?><?php echo mb_strlen((string)$c['body'], 'UTF-8')>140?'…':''; ?></div>
                <div class="text-muted small">IP: <?php echo h((string)($c['ip'] ?? '')); ?></div>
              </td>
              <td>
                <?php if ($newsUrl): ?>
                  <a href="<?php echo h($newsUrl); ?>" target="_blank" class="text-decoration-none"><?php echo h(mb_substr($newsTitle,0,80,'UTF-8')); ?></a>
                <?php else: ?>
                  <?php echo h($newsTitle); ?>
                <?php endif; ?>
              </td>
              <td>
                <div class="fw-semibold"><?php echo h((string)($c['name'] ?? '')); ?></div>
                <div class="text-muted small"><?php echo h((string)($c['email'] ?? '')); ?></div>
              </td>
              <td class="text-muted small">
                <?php echo h((string)($c['created_at'] ?? '')); ?>
              </td>
              <td>
                <form method="post" class="d-flex flex-wrap gap-1">
                  ...
                  <?php if ($st !== 'approved'): ?>
                    <button class="btn btn-sm btn-success" name="action" value="approve"><?php echo h(__('t_51cc4e6944', 'اعتماد')); ?></button>
                  <?php endif; ?>
                  <?php if ($st !== 'rejected'): ?>
                    <button class="btn btn-sm btn-warning" name="action" value="reject"><?php echo h(__('t_b7dee9747a', 'رفض')); ?></button>
                  <?php endif; ?>
                  <button class="btn btn-sm btn-outline-danger" name="action" value="delete" data-confirm='حذف التعليق نهائيًا؟'><?php echo h(__('t_3b9854e1bb', 'حذف')); ?></button>
                </form>
              </td>
            </tr>
          <?php endforeach; ?>
    ...
    <?php if ($totalPages > 1): ?>
      <div class="card-body border-top d-flex align-items-center justify-content-between">
        <div class="text-muted small">النتائج: <?php echo (int)$total; ?></div>
        <nav>
          <ul class="pagination pagination-sm mb-0">
            ...
            <li class="page-item <?php echo $page<=1?'disabled':''; ?>">
              <a class="page-link" href="<?php echo h($base . '&page=' . max(1,$page-1)); ?>"><?php echo h(__('t_650bd5b508', 'السابق')); ?></a>
            </li>
            <li class="page-item disabled"><span class="page-link"><?php echo (int)$page; ?> / <?php echo (int)$totalPages; ?></span></li>
            <li class="page-item <?php echo $page>=$totalPages?'disabled':''; ?>">
              <a class="page-link" href="<?php echo h($base . '&page=' . min($totalPages,$page+1)); ?>"><?php echo h(__('t_8435afd9e8', 'التالي')); ?></a>
            </li>
          </ul>
        </nav>
      </div>
