...
if ((string)gdy_get_server_raw('REQUEST_METHOD', 'GET') === 'POST') {
    ...
    if (!$error) {
        ...
        // نقرأ من جدول users
        // Compatibility: بعض قواعد البيانات القديمة تخزن الهاش في عمود `password` بدل `password_hash`.
        ...
        // بعض تعريفات PDO/Drivers قد ترمي HY093 عند تمرير مصفوفة بمفاتيح تحمل ":".
        // الحل الأكثر توافقًا: bindValue ثم execute().
...
if (!$user || !in_array($role, $allowedRoles, true)) {
    throw new Exception(__('t_81ba8a03a2', 'بيانات الدخول غير صحيحة أو لا تملك صلاحية الدخول للوحة التحكم.'));
        ...
        // Legacy: بعض النسخ كانت تستخدم `password` لتخزين bcrypt (وأحيانًا داخل <>).
