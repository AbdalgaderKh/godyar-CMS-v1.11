<?php
/* ✅ HOTFIX PHP8: gdy_dirname() WRAP FIX APPLIED (v4) */

if (!function_exists('gdy_dirname')) {
    /**
     * PHP 8+ safe dirname wrapper.
     * - gdy_dirname($path, 0) throws ValueError on PHP 8+, so we return $path as-is.
     * - For all other cases, behavior matches native gdy_dirname().
     */
...
$rootPath = __DIR__;
for ($i=0; $i<8; $i++) {
  $candidate = gdy_dirname($rootPath, $i);
  ...
  $GDY_ROOT = __DIR__;
  for ($i = 0; $i < 8; $i++) {
    $candidate = gdy_dirname(__DIR__, $i);
  ...
  if (isset($pdo) && $pdo instanceof PDO) {
    ...
    try { $pdoCandidate = \Godyar\DB::pdo(); } catch (Exception $e) { /* ignore */ }
  }
...
if (!function_exists('gdy_root_url')) {
    function gdy_root_url(): string {
        ...
        $dir = rtrim(str_replace('\\', '/', gdy_dirname($scriptName)), '/');
        if ($dir === '' || $dir === '.') { $dir = ''; }
...
<link rel="canonical" href="<?php echo u($seoUrl); ?>">
  ...
  <?php endif; ?>
    ...
    <link rel="stylesheet" href="<?php echo h(rtrim((string)($baseUrl ?? ''), '/')); ?>/assets/css/grid.css?v=20260210_1">

    <link rel="stylesheet" href="<?php echo h(rtrim((string)($baseUrl ?? ''), '/')); ?>/assets/css/grid-bootstrap-compat.css?v=20260211_1">
<link rel="stylesheet" href="<?php echo h(rtrim((string)($baseUrl ?? ''), '/')); ?>/assets/css/media-system.css?v=20260211_1">
...
<!-- Theme Core (Front): يجعل تغيير الثيم ينعكس على كل عناصر الواجهة بشكل احترافي -->
    ...
    <?php
      ...
      if ($themeFront !== 'default') {
        $themeCssDisk = gdy_dirname(__DIR__, 4) . '/assets/css/themes/theme-' . $themeFront . '.css';
