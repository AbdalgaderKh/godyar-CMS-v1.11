...
// Detect admin area early
$__reqUri = $_SERVER['REQUEST_URI'] ?? '/';
$__isAdminArea = (bool)preg_match('#^/admin(?:/|$)#', $__reqUri);

// Start session as early as possible (single, consistent cookie)
// NOTE: Using multiple session names (admin/public) caused CSRF/logout issues.
// Keep one session cookie site-wide.
if (session_status() !== PHP_SESSION_ACTIVE) {
    if (session_name() !== 'GDYSESSID') {
        @session_name('GDYSESSID');
    }
...
// Admin-only session guards (optional)
if ($__isAdminArea && function_exists('gdy_admin_session_guard')) {
    gdy_admin_session_guard();
}
...
// --- Ensure DB class is loaded (some hosts don't autoload classes) ---
if (!class_exists('\\Godyar\\DB') && is_file(ROOT_PATH . '/includes/classes/DB.php')) {
    require_once ROOT_PATH . '/includes/classes/DB.php';
}
    ...
    if (function_exists('error_log')) {
        if (!defined('GODYAR_BOOTSTRAP_PDO_MISSING_LOGGED')) {
            ...
            if (class_exists(PDO::class)) {
                try { $drivers = PDO::getAvailableDrivers(); } catch (Exception $e) { $drivers = []; }
            }
...
if (!headers_sent()) {
	...
	// Expose nonce to included templates (many templates echo $nonce)
	// so they don't output nonce="" which would be blocked by CSP.
	$GLOBALS['nonce'] = $nonce;

	// NOTE: Older attempts tried to post-process the entire HTML output via $html,
	// but $html isn't guaranteed to exist (no output buffering everywhere), which
	// caused fatal errors on PHP 8.4. Keep CSP enforcement via proper nonce usage
...
if ($isHtml && !$isApi && !$isAsset && PHP_SAPI !== 'cli') {
    ob_start(function ($html) {
        ...
        // Fix templates that render nonce="" / nonce='' (empty). CSP requires the
        // same nonce value as in the response header.
        $html = preg_replace_callback(
            '~<(script|style)\b([^>]*?)\bnonce=("|\')\s*\3([^>]*)>~i',
            function ($m) use ($nonce) {
                return '<' . $m[1] . $m[2] . ' nonce="' . $nonce . '"' . $m[4] . '>';
            },
            $html
        );
...
try {
    ...
    try {
        ...
        if ($emailForHydrate !== '' && $uidForHydrate <= 0 && isset($pdo) && $pdo instanceof \PDO) {
            // Determine best columns present in users table (old/new schemas)
    $dnCol = db_column_exists($pdo, 'users', 'display_name')
      ? 'display_name'
      : (db_column_exists($pdo, 'users', 'name') ? 'name' : (db_column_exists($pdo, 'users', 'full_name') ? 'full_name' : (db_column_exists($pdo, 'users', 'username') ? 'username' : null)));

    $emailCol = db_column_exists($pdo, 'users', 'email')
      ? 'email'
      : (db_column_exists($pdo, 'users', 'email_address') ? 'email_address' : null);

    $roleExpr = db_column_exists($pdo, 'users', 'role') ? 'role' : 'NULL AS role';

    $select = "id";
    if (db_column_exists($pdo, 'users', 'username')) { $select .= ", username"; }
    if ($dnCol) { $select .= ", {$dnCol} AS display_name"; }
    if ($emailCol) { $select .= ", {$emailCol} AS email"; }
    $select .= ", {$roleExpr}";

    // If we don't have an email column, fall back to matching by id
    $where = $emailCol ? "{$emailCol}=:e" : "id=:e";

    $stHyd = $pdo->prepare("SELECT {$select} FROM users WHERE {$where} LIMIT 1");
