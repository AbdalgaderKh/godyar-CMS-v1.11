...
// Hardened: if already installed, allow rerun ONLY with reinstall=1 AND a valid token.
$lockFile = __DIR__ . '/install.lock';
$envFile  = $ROOT . '/.env';
$storageLock = $ROOT . '/storage/install.lock';
$installed = is_file($lockFile);
function installer_read_env_token(string $envPath): string {
...
/", $raw) as $line) {
        ...
        if (stripos($line, 'INSTALL_REINSTALL_TOKEN=') === 0) {
            $v = substr($line, strlen('INSTALL_REINSTALL_TOKEN='));
            $v = trim($v);
...
if ($installed) {
    $wantReinstall = isset($_GET['reinstall']) && (string)$_GET['reinstall'] === '1';
    $token = isset($_GET['token']) ? (string)$_GET['token'] : '';
    $expected = installer_read_env_token($envFile);

    // If token exists, require it. If no token stored yet, allow reinstall=1.
    $tokenOk = ($expected === '') ? $wantReinstall : ($wantReinstall && hash_equals($expected, $token));

    if (!$tokenOk) {
        http_response_code(403);
        exit('Installer is disabled. Delete the /install directory. If you must rerun it, use /install/?reinstall=1' . ($expected !== '' ? '&token=YOUR_TOKEN (token is stored in .env as INSTALL_REINSTALL_TOKEN)' : '') . '.');
    }
}
...
if ($step === 3) {
    try {
        ...
        if ($drv !== 'pgsql') {
            ...
            try { $pdo->exec("SET client_encoding TO 'UTF8'"); } catch (Exception $e) {}
        }
        ...
        // 2c) Post-install schema verification (fast sanity check)
        function installer_table_exists(PDO $pdo, string $drv, string $table): bool {
            if ($drv === 'pgsql') {
        ...
        function installer_column_exists(PDO $pdo, string $drv, string $table, string $col): bool {
            if ($drv === 'pgsql') {
                $stmt = $pdo->prepare("SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name=:t AND column_name=:c LIMIT 1");
            } else {
        ...
        $expectedTables = [
            ...
            'categories','team_members','opinion_authors',
            'settings','site_settings','pages','tags','tag_meta',
            'news','news_tags','news_views','news_revisions','news_notes','news_audit_log',
            'comments','comment_votes',
            'sessions','visits','analytics_events',
            'password_resets','contact_messages','user_bookmarks',
            'news_reactions','settings_snapshots','feeds','featured_videos',
            'news_imports','news_import_items','news_questions',
            'news_translations',
            // Admin UX
            'admin_notifications','admin_saved_filters'
        ];

        $expectedColumns = [
            'users' => ['id','email','username','password_hash','role','is_admin','status'],
            'roles' => ['id','name'],
            'permissions' => ['id','slug'],
            'news' => ['id','title','slug','status'],
        ];

        $missingTables = [];
        foreach ($expectedTables as $t) {
            if (!installer_table_exists($pdo, $drv, $t)) { $missingTables[] = $t; }
        }

        $missingCols = [];
        foreach ($expectedColumns as $t => $cols) {
            if (!installer_table_exists($pdo, $drv, $t)) { continue; }
            foreach ($cols as $c) {
                if (!installer_column_exists($pdo, $drv, $t, $c)) {
                    $missingCols[] = $t . '.' . $c;
                }
            }
        }
        ...
        if ($missingTables || $missingCols) {
            ...
            if ($missingTables) { $msg .= "Missing tables: " . implode(', ', $missingTables) . "\\n"; }
            if ($missingCols)   { $msg .= "Missing columns: " . implode(', ', $missingCols) . "\\n"; }
...
// 3) Create admin user & assign admin role
        ...
        $reinstallToken = bin2hex(random_bytes(24));
        $env = [];
        ...
        $env[] = env_line('INSTALL_REINSTALL_TOKEN',$reinstallToken);
        $env[] = env_line('GDY_INSTALLED','1');
        $env[] = '';
...
", LOCK_EX);
        ...
        $body .= '<div class="warn"><b>مهم:</b> احفظ توكن إعادة تشغيل المُثبّت (لن يظهر مرة أخرى):<br><code>'.h($reinstallToken).'</code><br>بعد التأكد من عمل الموقع احذف مجلد <code>/install</code> فوراً.</div>';
