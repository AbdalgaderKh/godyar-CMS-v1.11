...
if ($pdo instanceof PDO) {
    try {
        $roleExpr = (function_exists('db_column_exists') && db_column_exists($pdo, 'team_members', 'role'))
            ? 'role'
            : ((function_exists('db_column_exists') && db_column_exists($pdo, 'team_members', 'position')) ? 'position' : 'NULL');

        $photoExpr = (function_exists('db_column_exists') && db_column_exists($pdo, 'team_members', 'photo_url'))
            ? 'photo_url'
            : ((function_exists('db_column_exists') && db_column_exists($pdo, 'team_members', 'image')) ? 'image' : 'NULL');

        $roleExpr = (function_exists('db_column_exists') && db_column_exists($pdo, 'team_members', 'role'))
            ? 'role'
            : ((function_exists('db_column_exists') && db_column_exists($pdo, 'team_members', 'position')) ? 'position' : "''");

$sql = "
  SELECT
    id,
    name,
    {$roleExpr} AS position,
    image,
    bio,
    facebook,
    twitter,
