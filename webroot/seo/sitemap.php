...
if (($nocache === false) && is_file($cacheFile) && (time() - filemtime($cacheFile) < $cacheTtl)) {
...
if ($pdo instanceof PDO) {
    try {
        ...
        $cats = (empty($st) === false) ? $st->fetchAll(PDO::FETCH_ASSOC) : [];
        foreach ($cats as $c) {
...
if ($pdo instanceof PDO) {
    try {
        ...
        $rows = (empty($st) === false) ? $st->fetchAll(PDO::FETCH_ASSOC) : [];
        foreach ($rows as $r) {
            $id = (int)($r['id'] ?? 0);
            if ($id <= 0) continue;
            $lm = '';
...
echo '</urlset>';
$xml = ob_get_clean();
if (is_string($xml) && (empty($xml) === false) !== '') {
